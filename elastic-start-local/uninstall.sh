...
if ask_confirmation; then
  ...
  echo "- docker.elastic.co/elasticsearch/elasticsearch:9.1.2"
  if ask_confirmation; then
    if docker rmi "docker.elastic.co/elasticsearch/elasticsearch:9.1.2" >/dev/null 2>&1; then
      echo "Image docker.elastic.co/elasticsearch/elasticsearch:9.1.2 removed successfully"
    else
      echo "Failed to remove image docker.elastic.co/elasticsearch/elasticsearch:9.1.2. It might be in use."
    fi
  fi
